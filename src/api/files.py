...
async def upload_file(file: Annotated[UploadFile, File(..., description='Загружаемый файл')]):
    async with aiofiles.open(f'{settings.base_dir}/{settings.dump_path}/upload_{file.filename}', 'wb') as f:
...
async def upload_files(files: Annotated[List[UploadFile], File(..., description='Загружаемый файл')]):
    for file in files:
        async with aiofiles.open(f'{settings.base_dir}/{settings.dump_path}/1_{file.filename}', 'wb') as f:
            await f.write(await file.read())


@router_files.get('/files/get/{filename}', tags=['Работа с файлами'], summary='Выгрузка с сервера файла целиком')
async def get_file(filename: str):
    if os.path.exists(f'{settings.base_dir}/{settings.dump_path}/{filename}'):
        return FileResponse(f'{settings.base_dir}/{settings.dump_path}/{filename}')
...
def generation_chunks(filename):
    with open(f'{settings.base_dir}/{settings.dump_path}/{filename}', 'rb') as file:
